...
METHOD_MATCHER = re.compile(r"def agent_v\d\(.*?\) -> float:(?:\s*(?:[ \t]*(?!def|#|`|').*(?:\n|$)))+")
...
class DummySandbox:
  ...
  def compile_code(program: str):
    ...
    #print(f'program in compiple_code: {program}')
    namespace = {}
  ...
  def run(
    ...
    try:
      ...
      with open(prog_file, "wb+") as f:
        '''
        print(f'function to run in cloudpickle: {function_to_run}') 
        print(f'namespace[function_to_run], f: {namespace[function_to_run]}, {f}') 
        '''
        cloudpickle.dump(namespace[function_to_run], f)

      error_file = self.output_path / f"stderr_{self.call_count}.log"

      print(f'error file path: {error_file}') 
